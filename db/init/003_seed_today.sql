INSERT INTO payments_in_success (
  event_trace_id,
  account_number,
  customer_type,
  event_datetime,
  source_topic,
  source_partition_id,
  source_offset,
  message_key,
  source_payload,
  transformed_payload,
  latency_ms,
  latency_event_received_ms,
  latency_event_sent_ms,
  target_topic,
  target_partition_id,
  target_offset
) VALUES
  ('trace-pay-101', 'ACC-3001', 'RETAIL', TIMESTAMP(CURDATE(), '08:05:00'),
   'payments.raw', 1, 110, 'pay-key-101', '{"amount":120}', '{"amount":120,"status":"ok"}',
   4000, 1500, 2500, 'payments.processed', 2, 210),
  ('trace-pay-102', 'ACC-3002', 'SMB', TIMESTAMP(CURDATE(), '09:20:00'),
   'payments.raw', 1, 111, 'pay-key-102', '{"amount":240}', '{"amount":240,"status":"ok"}',
   6000, 2500, 3500, 'payments.processed', 2, 211),
  ('trace-pay-103', 'ACC-3003', 'RETAIL', TIMESTAMP(CURDATE(), '10:15:00'),
   'payments.raw', 1, 112, 'pay-key-103', '{"amount":65}', '{"amount":65,"status":"ok"}',
   3000, 1200, 1800, 'payments.processed', 2, 212);

INSERT INTO payments_in_failure (
  event_trace_id,
  account_number,
  customer_type,
  event_datetime,
  source_topic,
  source_partition_id,
  source_offset,
  message_key,
  source_payload,
  transformed_payload,
  latency_ms,
  latency_event_received_ms,
  target_topic,
  target_partition_id,
  target_offset,
  exception_type,
  exception_message,
  exception_stack,
  retriable,
  retry_attempt
) VALUES
  ('trace-pay-f-101', 'ACC-3009', 'SMB', TIMESTAMP(CURDATE(), '11:05:00'),
   'payments.raw', 1, 113, 'pay-key-104', '{"amount":90}', '{"amount":90}',
   5000, 5000, 'payments.processed', 2, 213,
   'TimeoutException', 'Downstream timeout', 'stack...', 1, 0);

INSERT INTO loans_in_success (
  event_trace_id,
  account_number,
  customer_type,
  event_datetime,
  source_topic,
  source_partition_id,
  source_offset,
  message_key,
  source_payload,
  transformed_payload,
  latency_ms,
  latency_event_received_ms,
  latency_event_sent_ms,
  target_topic,
  target_partition_id,
  target_offset
) VALUES
  ('trace-loan-101', 'ACC-4001', 'RETAIL', TIMESTAMP(CURDATE(), '08:35:00'),
   'loans.raw', 3, 510, 'loan-key-101', '{"loanAmount":15000}', '{"loanAmount":15000,"status":"ok"}',
   9000, 3500, 5500, 'loans.processed', 4, 610),
  ('trace-loan-102', 'ACC-4002', 'SMB', TIMESTAMP(CURDATE(), '12:10:00'),
   'loans.raw', 3, 511, 'loan-key-102', '{"loanAmount":6500}', '{"loanAmount":6500,"status":"ok"}',
   8000, 3000, 5000, 'loans.processed', 4, 611);

INSERT INTO loans_in_failure (
  event_trace_id,
  account_number,
  customer_type,
  event_datetime,
  source_topic,
  source_partition_id,
  source_offset,
  message_key,
  source_payload,
  transformed_payload,
  latency_ms,
  latency_event_received_ms,
  target_topic,
  target_partition_id,
  target_offset,
  exception_type,
  exception_message,
  exception_stack,
  retriable,
  retry_attempt
) VALUES
  ('trace-loan-f-101', 'ACC-4009', 'RETAIL', TIMESTAMP(CURDATE(), '13:25:00'),
   'loans.raw', 3, 512, 'loan-key-103', '{"loanAmount":2400}', '{"loanAmount":2400}',
   5000, 5000, 'loans.processed', 4, 612,
   'ValidationException', 'Loan data invalid', 'stack...', 0, 1);

INSERT INTO cards_in_success (
  event_trace_id,
  account_number,
  customer_type,
  event_datetime,
  source_topic,
  source_partition_id,
  source_offset,
  message_key,
  source_payload,
  transformed_payload,
  latency_ms,
  latency_event_received_ms,
  latency_event_sent_ms,
  target_topic,
  target_partition_id,
  target_offset
) VALUES
  ('trace-card-101', 'ACC-5001', 'RETAIL', TIMESTAMP(CURDATE(), '07:55:00'),
   'cards.raw', 2, 310, 'card-key-101', '{"cardType":"debit"}', '{"cardType":"debit","status":"ok"}',
   4000, 1500, 2500, 'cards.processed', 2, 410),
  ('trace-card-102', 'ACC-5002', 'SMB', TIMESTAMP(CURDATE(), '14:40:00'),
   'cards.raw', 2, 311, 'card-key-102', '{"cardType":"credit"}', '{"cardType":"credit","status":"ok"}',
   6000, 2500, 3500, 'cards.processed', 2, 411);

INSERT INTO cards_in_failure (
  event_trace_id,
  account_number,
  customer_type,
  event_datetime,
  source_topic,
  source_partition_id,
  source_offset,
  message_key,
  source_payload,
  transformed_payload,
  latency_ms,
  latency_event_received_ms,
  target_topic,
  target_partition_id,
  target_offset,
  exception_type,
  exception_message,
  exception_stack,
  retriable,
  retry_attempt
) VALUES
  ('trace-card-f-101', 'ACC-5009', 'RETAIL', TIMESTAMP(CURDATE(), '15:05:00'),
   'cards.raw', 2, 312, 'card-key-103', '{"cardType":"debit"}', '{"cardType":"debit"}',
   4000, 4000, 'cards.processed', 2, 412,
   'IllegalStateException', 'Card status invalid', 'stack...', 1, 0);

INSERT INTO accounts_in_success (
  event_trace_id,
  account_number,
  customer_type,
  event_datetime,
  source_topic,
  source_partition_id,
  source_offset,
  message_key,
  source_payload,
  transformed_payload,
  latency_ms,
  latency_event_received_ms,
  latency_event_sent_ms,
  target_topic,
  target_partition_id,
  target_offset
) VALUES
  ('trace-acc-101', 'ACC-6001', 'RETAIL', TIMESTAMP(CURDATE(), '06:40:00'),
   'accounts.raw', 5, 710, 'acc-key-101', '{"type":"checking"}', '{"type":"checking","status":"ok"}',
   5000, 2000, 3000, 'accounts.processed', 5, 810),
  ('trace-acc-102', 'ACC-6002', 'SMB', TIMESTAMP(CURDATE(), '09:50:00'),
   'accounts.raw', 5, 711, 'acc-key-102', '{"type":"savings"}', '{"type":"savings","status":"ok"}',
   7000, 2500, 4500, 'accounts.processed', 5, 811);

INSERT INTO accounts_in_failure (
  event_trace_id,
  account_number,
  customer_type,
  event_datetime,
  source_topic,
  source_partition_id,
  source_offset,
  message_key,
  source_payload,
  transformed_payload,
  latency_ms,
  latency_event_received_ms,
  target_topic,
  target_partition_id,
  target_offset,
  exception_type,
  exception_message,
  exception_stack,
  retriable,
  retry_attempt
) VALUES
  ('trace-acc-f-101', 'ACC-6009', 'RETAIL', TIMESTAMP(CURDATE(), '11:45:00'),
   'accounts.raw', 5, 712, 'acc-key-103', '{"type":"checking"}', '{"type":"checking"}',
   4000, 4000, 'accounts.processed', 5, 812,
   'ValidationException', 'Account data missing', 'stack...', 0, 1);

INSERT INTO transfers_in_success (
  event_trace_id,
  account_number,
  customer_type,
  event_datetime,
  source_topic,
  source_partition_id,
  source_offset,
  message_key,
  source_payload,
  transformed_payload,
  latency_ms,
  latency_event_received_ms,
  latency_event_sent_ms,
  target_topic,
  target_partition_id,
  target_offset
) VALUES
  ('trace-trf-101', 'ACC-7001', 'RETAIL', TIMESTAMP(CURDATE(), '10:05:00'),
   'transfers.raw', 6, 910, 'trf-key-101', '{"amount":500}', '{"amount":500,"status":"ok"}',
   5000, 2000, 3000, 'transfers.processed', 6, 1010),
  ('trace-trf-102', 'ACC-7002', 'SMB', TIMESTAMP(CURDATE(), '16:20:00'),
   'transfers.raw', 6, 911, 'trf-key-102', '{"amount":900}', '{"amount":900,"status":"ok"}',
   7000, 2500, 4500, 'transfers.processed', 6, 1011);

INSERT INTO transfers_in_failure (
  event_trace_id,
  account_number,
  customer_type,
  event_datetime,
  source_topic,
  source_partition_id,
  source_offset,
  message_key,
  source_payload,
  transformed_payload,
  latency_ms,
  latency_event_received_ms,
  target_topic,
  target_partition_id,
  target_offset,
  exception_type,
  exception_message,
  exception_stack,
  retriable,
  retry_attempt
) VALUES
  ('trace-trf-f-101', 'ACC-7009', 'SMB', TIMESTAMP(CURDATE(), '17:35:00'),
   'transfers.raw', 6, 912, 'trf-key-103', '{"amount":1200}', '{"amount":1200}',
   6000, 6000, 'transfers.processed', 6, 1012,
   'TimeoutException', 'Transfer timeout', 'stack...', 1, 0);

INSERT INTO alerts_in_success (
  event_trace_id,
  account_number,
  customer_type,
  event_datetime,
  source_topic,
  source_partition_id,
  source_offset,
  message_key,
  source_payload,
  transformed_payload,
  latency_ms,
  latency_event_received_ms,
  latency_event_sent_ms,
  target_topic,
  target_partition_id,
  target_offset
) VALUES
  ('trace-alt-101', 'ACC-8001', 'RETAIL', TIMESTAMP(CURDATE(), '05:30:00'),
   'alerts.raw', 7, 1110, 'alt-key-101', '{"type":"fraud"}', '{"type":"fraud","status":"ok"}',
   3000, 1200, 1800, 'alerts.processed', 7, 1210),
  ('trace-alt-102', 'ACC-8002', 'SMB', TIMESTAMP(CURDATE(), '18:15:00'),
   'alerts.raw', 7, 1111, 'alt-key-102', '{"type":"latency"}', '{"type":"latency","status":"ok"}',
   5000, 2000, 3000, 'alerts.processed', 7, 1211);

INSERT INTO alerts_in_failure (
  event_trace_id,
  account_number,
  customer_type,
  event_datetime,
  source_topic,
  source_partition_id,
  source_offset,
  message_key,
  source_payload,
  transformed_payload,
  latency_ms,
  latency_event_received_ms,
  target_topic,
  target_partition_id,
  target_offset,
  exception_type,
  exception_message,
  exception_stack,
  retriable,
  retry_attempt
) VALUES
  ('trace-alt-f-101', 'ACC-8009', 'RETAIL', TIMESTAMP(CURDATE(), '19:05:00'),
   'alerts.raw', 7, 1112, 'alt-key-103', '{"type":"fraud"}', '{"type":"fraud"}',
   4000, 4000, 'alerts.processed', 7, 1212,
   'RuntimeException', 'Alert dispatch failed', 'stack...', 0, 1);

